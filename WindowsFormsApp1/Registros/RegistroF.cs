    ...
    public partial class RegistroF : Form
    {
        ...
        private void btnRegisterF_Click(object sender, EventArgs e)
        {
                ...
                !string.IsNullOrWhiteSpace(txtPWCF.Text))
            {
                ...
                string queryCadastroFornecedor = @"
            INSERT INTO CadastroFornecedor (nomeCompleto, idade, email, telefone, cpf, cnpj, senha) 
            VALUES (@nomeCompleto, @idade, @Email, @telefone, @cpf, @cnpj, @senha);
            SELECT SCOPE_IDENTITY();"; 

                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    SqlCommand command = new SqlCommand(queryCadastroFornecedor, connection);
                    ...
                    try
                    {
                        ...
                        int idCadastroFornecedor = Convert.ToInt32(command.ExecuteScalar());

                       
                        string queryFornecedor = @"
                    INSERT INTO Fornecedor (IDCadastroFornecedor) 
                    VALUES (@IDCadastroFornecedor)";

                        SqlCommand commandFornecedor = new SqlCommand(queryFornecedor, connection);
                        commandFornecedor.Parameters.AddWithValue("@IDCadastroFornecedor", idCadastroFornecedor);
